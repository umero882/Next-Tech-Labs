...
const tabs = [
  { to: '/projects/first-bite/support', label: 'Support' },
    ...
    <div className="min-h-screen">
      <section className="relative overflow-hidden border-b border-border">
        ...
        <Container className="relative pt-12 md:pt-16 pb-10">
          ...
          {subtitle && (
            <p className="mt-4 text-text-secondary text-lg leading-relaxed max-w-2xl">{subtitle}</p>
          )}

          {effective && <p className="mt-4 label-mono text-text-muted tabular">{effective}</p>}
          ...
          <nav aria-label="First Bite legal pages" className="mt-10 flex flex-wrap gap-2">
            {tabs.map((t) => {
      ...
      <Container className="py-14 md:py-20 max-w-3xl">
        ...
        <footer className="mt-16 pt-8 border-t border-border flex flex-wrap items-center justify-between gap-4 text-sm text-text-muted">
          <p className="tabular">© {new Date().getFullYear()} Next Tech Labs · First Bite</p>
          <a
